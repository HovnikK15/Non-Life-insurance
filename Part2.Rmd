...
Firs we need to import our data from the file `SeverityCensoring` into R. The file contains 9062 claims, with columns representing policy id, claim id, rc (iz claim is censored - number or uncensored - value NA ), deductible (always 100) and the claim amount.
...
Now we can finally fit exponention distribution to our model.We will compute negative log-likelihood function, taking trucation and censoring into account. We will do this with the help of `dexp` and `pexp` function that represent density and distribution functions of exponential distribution. As our paramether $\lambda > 0$ we use $\lambda = e^{\beta}$  and now it can take all values.   
```{r, warning=FALSE}
...
```
To get the enitial estimation of our parameter, we use the moment estimation. As we now $$ E(X) = \frac1\lambda $$ and from here we get that $$ \lambda = \frac1{m_1} $$. Where $m_1$ is first moment derived from our data. Now we can fit exponential distribution and we get the MLE estimation of $\lambda$:
```{r, echo = F, warning=FALSE}
...
We now get first and the second moment from our data and from that our estimations. 
We fit the model end get our estimates for $\mu$ and $\sigma^2$:
```{r, echo = F, warning=FALSE}
...
# 2.2 Fitting inverse Gaussian distributio
...
Here we need to definethree vectors: deds, loss and full and then plot the fit. We do this like this:
```{r, warning=FALSE}
...
# 5 Plots
Now we will plot all the survival functions for all the different distributions onto one graph, so that we can sompare them. The graph looks like this.
```{r, echo = F, warning=FALSE}
